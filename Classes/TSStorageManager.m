...
//            [db open];
            ...
            for (NSString* sql in sqls) {
                result = [db executeUpdate:sql withParameterDictionary:@{}];
                if (!result) {
                    NSLog(@"%@,==%@",  sql,[db lastErrorMessage]);
//                     [[UIApplication sharedApplication].keyWindow makeToast:[NSString stringWithFormat:@"%@,==%@",  sql,[db lastErrorMessage]] duration:3 position:CSToastPositionCenter];
                    break ;
                }
            }
            if (!result) {
 ...
 *  @param class
 */
-(BOOL)checkTableExists:(Class )class
{
    BOOL isexit;
       ...
       strOrderby:(NSString *)strOrderby
{
    ...
    NSString * querySql = [SqlBuilder buildQuerySql:class columns:nil];
    NSMutableArray * results =[[NSMutableArray alloc] init];
...
// MARK:  获取即将要生成的fmdbid用作字表的外键
- (NSInteger) getNewfmdbId:(Class )class
{
    TableInfo * tableInfo =     [TableInfo getTableInfo:class];
    NSString *sql = [NSString stringWithFormat:@"Select seq From sqlite_sequence where name  = '%@'",tableInfo.getTableName];
    
   __block NSInteger newfmdbid = 0;
    [self.dbQueue inDatabase:^(FMDatabase *db) {
//        [db open];
        FMResultSet *rs =[db executeQuery:sql];
        while ([rs next]){
             newfmdbid = [rs intForColumn:@"seq"];
 ...
 *
 */
-(BOOL)isDataExists:(id)entity{
    TableInfo * tableInfo =     [TableInfo getTableInfo:[entity class]];
    
    NSString *sql = [NSString stringWithFormat:@"select count(*) as 'count' from %@ where  %@ = '%@'",tableInfo.getTableName,tableInfo.getPrimaryFieldName,[entity valueForKey:tableInfo.getPrimaryFieldName]];
